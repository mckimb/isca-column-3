...
! ---------------------------------------------------------------------------------- !
! QL, Why i think this should be 1-w_free?  Sorry, it's explained above.
  DO l=1, n_profile
    IF (w_free(l, n_cloud_top) > tol_cloud) THEN
      tot_cloud_cover(l)=w_free(l, n_cloud_top) ! QL, 1-w_free???
    ELSE
      tot_cloud_cover(l) = 0.0e+00_RealK ! QL, 1.0??
    END IF
  END DO
...
!     atmospheric column. The area of common overlap between
