...
module qe_moist_convection_mod
  ...
  real    :: scaling = 1. ! prefactor in front of dtdlnp added by Brett on 10/12/21.
  real    :: tau_bm  = 7200.
  real    :: rhbm    = .8
  ...
  namelist /qe_moist_convection_nml/  tau_bm, rhbm, Tmin, Tmax, val_inc, scaling
...
10     call close_file(unit)
  ...
  subroutine qe_moist_convection (dt, Tin, qin, p_full, p_half, coldT, &
                                  ...
                                  invtau_t_relaxation, Tref, preslcl, ind_lcl, preslzb,pshallow,shallower_flag,precip_level_DNE_ktop_flag, precip_level_DNE_flag, deep_gorman_flag,deep_frierson_flag,noconvflag,precip_both_negative_flag) ! NTL ADDED preslcl as output ! BAM ADDED preslzb as output
    ...
    !           shallower_flag   Flag indicating if shallow convection occured
    !           precip_level_DNE_ktop_flag Flag indicating if one type of null convection occurred
    !           precip_level_DNE_flag Flag indicating if one type of null convection occurred
    !           deep_gorman_flag    Flag indicating if deep gorman convection occurred
    !           deep_frierson_flag Flag indicating if deep frierson convection occured
    !           noconvflag  Flag indicating if no convection occurred (CAPE<0)
    !           precip_both_negative_flag Flag indicating if shallow convection occured, but convection scheme did nothing
    ...
    real   , intent(in) , dimension(:,:,:) :: Tin, qin, p_full, p_half ! Brett changed 'tin' to 'Tin' here on 3 March 2022.
    real   , intent(in)                    :: dt
    logical, intent(in) , dimension(:,:)   :: coldT
    real   , intent(out), dimension(:,:)   :: rain, snow, kLZBs, CAPE, CIN, preslcl, ind_lcl, preslzb, pshallow ! NTL define preslcl here ! BAM define preslzb here
    real   , intent(out), dimension(:,:)   :: invtau_q_relaxation, invtau_t_relaxation
    integer, intent(out), dimension(:,:)   :: convflag, shallower_flag, precip_level_DNE_ktop_flag, precip_level_DNE_flag, deep_gorman_flag, deep_frierson_flag, noconvflag, precip_both_negative_flag
    ...
    call SBM_convection_scheme(dt, Tin, qin, p_full, p_half, rain, snow, &
         ...
         val_min, val_max, val_inc, lcl_temp_table, convflag, preslcl, ind_lcl, preslzb, pshallow,shallower_flag,precip_level_DNE_ktop_flag, precip_level_DNE_flag,deep_gorman_flag,deep_frierson_flag,noconvflag,precip_both_negative_flag) !  NTL added preslcl as output ! BAM added preslzb as output
  ...
  subroutine SBM_convection_scheme(dt, Tin, qin, p_full, p_half, rain,  snow, &
       ...
       Tref, qref, val_min, val_max, val_inc, lcl_temp_table, convflag, preslcl, ind_lcl, preslzb,pshallow,shallower_flag,precip_level_DNE_ktop_flag, precip_level_DNE_flag,deep_gorman_flag,deep_frierson_flag,noconvflag,precip_both_negative_flag) !NTL added preslcl as output ! BAM added preslzb as output
    ...
    real, intent(out), dimension(:,:)      :: preslcl, ind_lcl, presLZB, pshallow ! added by Brett ! added by BAM
    real, intent(out), dimension(:,:)      :: invtau_q_relaxation, invtau_t_relaxation
    ...
    ! real                                   :: cape_parcel, cin_parcel, Pq_parcel, Pt_parcel, preslcl_parcel, preslzb_parcel, pshallow_parcel, shallower_flag_parcel, precip_level_DNE_ktop_flag_parcel, precip_level_DNE_flag_parcel, deep_gorman_flag_parcel, deep_frierson_flag_parcel ! added by Brett ! added by BAM
    real                                   :: invtau_q_relaxation_parcel, invtau_t_relaxation_parcel
    real                                   :: cape_parcel, cin_parcel, Pq_parcel, Pt_parcel, preslcl_parcel, ind_lcl_parcel, preslzb_parcel, pshallow_parcel ! added by Brett ! added by BAM
!    real                                   :: invtau_q_relaxation_parcel, invtau_t_relaxation_parcel
    ...
    do i=1, size(Tin, 1)
       do j=1, size(Tin, 2)
          ...
          convflag(i,j) = 0
          shallower_flag(i,j) = 0
          precip_level_DNE_ktop_flag(i,j) = 0
          precip_level_DNE_flag(i,j) = 0
          deep_gorman_flag(i,j) = 0
          deep_frierson_flag(i,j) = 0
          noconvflag(i,j) = 0
          precip_both_negative_flag(i,j) = 0
          ...
          call CAPE_calculation(k_surface, p_full(i,j,:), p_half(i,j,:), &
               ...
               preslcl_parcel, ind_lcl_parcel, preslzb_parcel) ! NTL add preslcl output ! BAM added preslzb output
          ...
          if (cape_parcel .gt. 0) then
             ...
             if ( (Pq_parcel .gt. 0) .and. (Pt_parcel .gt. 0) ) then
                ...
                ! deep_gorman_flag(i,j) = 1 ! deep moist convection occurred
                ! deep_frierson_flag(i,j) = 1
                 ...
                 call do_deep_convection (kLZB, k_surface,Pt_parcel, dt,p_half(i,j,:),&
                     ...
                     deltaT_parcel,Tref_parcel,deltaq_parcel, deep_gorman_flag(i,j), deep_frierson_flag(i,j))

             else
                ! Else, if Pq <= 0 and Pt > 0
                if (Pt_parcel .gt. 0) then
                   ! shallower_flag(i,j) = 1 ! shallow convection occurred
                   ! DO shallow convection
                   call do_shallow_convection(kLZB, k_surface,qin(i,j,:),   &
                        qref_parcel, deltaq_parcel, Tin(i,j,:), Tref_parcel, &
                        deltaT_parcel, p_half(i,j,:), Pq_parcel,dt,pshallow_parcel,shallower_flag(i,j), precip_level_DNE_ktop_flag(i,j), precip_level_DNE_flag(i,j))
                   ! DONT shallow convection Added by BAM
                   !Pq_parcel = 0.
                   !call set_profiles_to_full_model_values (1, k_surface, Tin(i,j,:), qin(i,j,:), Tref_parcel, deltaT_parcel, qref_parcel, deltaq_parcel)

                else
                   ! Else, do nothing, and go back to loop over latitude and longitude
                   precip_both_negative_flag(i,j) = 1
                   Pq_parcel     = 0.
          ...
          Pq(i,j)       = Pq_parcel
          preslcl(i,j) = preslcl_parcel ! added by Brett !NTL just put this into preslcl
          ind_lcl(i,j) = ind_lcl_parcel
          preslzb(i,j) = preslzb_parcel ! BAM just put this into preslzb
          pshallow(i,j) = pshallow_parcel
    ...
    rain = Pq
    snow = 0.
    !preslcl = preslcl_array ! added by Brett ! NTL don't need this as doing it above now
  end subroutine sbm_convection_scheme

  !#######################################################################

  subroutine CAPE_calculation(k_surface, p_full, p_half, Tin, rin, kLZB, &
       Tp, rp, CAPE, CIN, val_min, val_max, lcl_temp_table, preslcl, ind_lcl, preslzb) ! NTL add preslcl as output ! BAM added preslzb as output
    ...
    real, intent(out)                    :: preslcl, ind_lcl, preslzb ! added by Brett ! added by BAM
    ...
    kLFC   = 0
    kLZB   = 0
    preslcl= 0. ! added by Brett
    ind_lcl=0
    preslzb= 0. ! added by BAM
    Tp     = Tin
    rp     = rin
    saturated = .false.
    ...
    ind_lcl = kLCL ! add the +1 because CAPE_below_LCL subtracts 1 after the pLCL calculation for some reason
    preslcl = pLCL ! added by Brett
    preslzb = pLZB ! added by BAM
...
20  end if
  ...
  subroutine do_shallow_convection(kLZB, k_surface, qin, qref, deltaq, Tin, &
       Tref, deltaT, p_half, Pq,dt,pshallow,shallower_flag, precip_level_DNE_ktop_flag, precip_level_DNE_flag)
    ...
    real, intent(out)                      :: pshallow
    integer, intent(out)                   :: shallower_flag
    integer, intent(out)                   :: precip_level_DNE_ktop_flag
    integer, intent(out)                   :: precip_level_DNE_flag
    ...
    call level_of_zero_precip(kLZB, k_surface, deltaq, p_half, Pq,  &
         k_zero_precip_found, k_top, deltaT, Tref, qref, Tin, qin,pshallow)
  ...
  subroutine level_of_zero_precip (kLZB, k_surface, deltaq, p_half, Pq,    &
       k_zero_precip_found, k_zero_precip, deltaT, Tref, qref, Tin, qin,pshallow)
  ...
  subroutine do_deep_convection (kLZB, k_surface,Pt, dt,p_half,invtau_q_relaxation, &
                                 invtau_t_relaxation,Pq,deltaT,Tref,deltaq,deep_gorman_flag, deep_frierson_flag)
    ...
    real, intent(inout), dimension(:)      :: deltaq
    integer, intent(out)                   :: deep_gorman_flag, deep_frierson_flag
    real, intent(out)                      :: invtau_q_relaxation, invtau_t_relaxation
    ! write(6,*) 'qe kLZB = ', kLZB
    ...
    if (Pq.gt.Pt)then
       ...
       ! call do_change_Tref_deepconv(kLZB, k_surface, deltaT, deltaq, p_half, Tref,dt) ! don't do ogorman, do frierson always for deep convection
       deep_gorman_flag = 1
