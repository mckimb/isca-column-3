...
10     call close_file(unit)
  ...
  subroutine SBM_convection_scheme(dt, Tin, qin, p_full, p_half, rain,  snow, &
    ...
    real, intent(out), dimension(:,:)      :: preslcl ! added by Brett
    real, intent(out), dimension(:,:)      :: invtau_q_relaxation, invtau_t_relaxation
    ...
    real, dimension(size(Tin, 1), size(Tin, 2))            :: Pq, preslcl_array ! added by Brett
    real, dimension(size(Tin,1), size(Tin,2), size(Tin,3)) :: rin
    real                                   :: cape_parcel, cin_parcel, Pq_parcel, Pt_parcel, preslcl_parcel ! added by Brett
    ...
    do i=1, size(Tin, 1)
       do j=1, size(Tin, 2)
          ...
          Pq(i,j)       = Pq_parcel
          preslcl_array(i,j) = preslcl_parcel ! added by Brett
    ...
    rain = Pq
    snow = 0.
    preslcl = preslcl_array ! added by Brett
  ...
  subroutine CAPE_calculation(k_surface, p_full, p_half, Tin, rin, kLZB, &
    ...
    real, intent(out)                    :: preslcl ! added by Brett
