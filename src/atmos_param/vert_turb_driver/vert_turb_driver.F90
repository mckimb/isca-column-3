...
use    diffusivity_mod, only: diffusivity, molecular_diff, diffusivity_lcl
...
public   vert_turb_driver_init, vert_turb_driver_end, vert_turb_driver, vert_turb_driver_lcl
...
subroutine vert_turb_driver_lcl (is, js, Time, Time_next, dt, tdtlw,     &
                             frac_land,   &
                             p_half, p_full, z_half, z_full, u_star,   &
                             b_star, q_star, rough, lat, convect,      &
                             u, v, t, q, r, um, vm, tm, qm, rm,        &
                             udt, vdt, tdt, qdt, rdt, diff_t, diff_m,  &
                             gust, z_pbl, ind_lcl, mask, kbot                  )

!-----------------------------------------------------------------------
integer,         intent(in)         :: is, js
type(time_type), intent(in)         :: Time, Time_next
   real,         intent(in)         :: dt
   ! real,         intent(inout)      :: preslcl
   real, intent(in), dimension(:,:) :: frac_land, u_star, b_star,  &
                                       q_star, rough, lat
logical, intent(in), dimension(:,:) :: convect
   real, intent(in), dimension(:,:,:) :: tdtlw, p_half, p_full, &
   ...
   real, intent(in) ,   dimension(:,:,:,:) :: r, rm, rdt
   real, intent(out),   dimension(:,:,:) :: diff_t, diff_m
   real, intent(out),   dimension(:,:)   :: gust, z_pbl,ind_lcl
   real, intent(in),optional, dimension(:,:,:) :: mask
integer, intent(in),optional, dimension(:,:) :: kbot
!-----------------------------------------------------------------------
real   , dimension(size(t,1),size(t,2),size(t,3))   :: ape, thv
...
!------------------- non-local K scheme --------------
    ...
    call diffusivity_lcl ( tt, qq, uu, vv, p_full, p_half, z_full, z_half,   &
                       u_star, b_star, z_pbl, ind_lcl, diff_m, diff_t, &
                       kbot = kbot)
...
end subroutine vert_turb_driver_lcl
