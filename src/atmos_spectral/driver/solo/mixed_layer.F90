...
real    :: sst_pert = 0. ! perturb the APE SST by a constant amount
...
namelist/mixed_layer_nml/ evaporation, depth, qflux_amp, qflux_width, tconst,&
                              ...
                              flux_lhe_anom_field_name, do_ape_sst, qflux_field_name, sst_pert
...
if (do_ape_sst) then
    ...
    do j=js,je
        if ( (rad_lat_2d(1,j) .gt. -PI/3.) .and. (rad_lat_2d(1,j) .lt. PI/3.) ) then
            ! between 60N-60S
            sst_new(:,j) = KELVIN+( 27.0*( 1. - (sin( 3./2. * rad_lat_2d(:,j) )**2 ) ) + sst_pert)
            !write(6,*) 'SST profile', rad_lat_2d(1,j)*180/PI, ' j:', j, ' sst:', sst_new(1,j)
        else
            ! from 60N/S to pole
            sst_new(:,j) = KELVIN + sst_pert
