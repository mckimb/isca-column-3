...
!if(lwet_convection .or. do_bm) then
   ...
   id_preslcl = register_diag_field(mod_name, 'presLCL',            &
        axes(1:2), Time, 'Lifting condensation level','hPa') ! added by Brett
...
rain = 0.0; snow = 0.0; precip = 0.0; preslcl = 0.0;
...
!  note the delta's are returned rather than the time derivatives
   ...
   rain       = rain/delta_t
   precip     = rain
   preslcl    = preslcl ! added by Brett
